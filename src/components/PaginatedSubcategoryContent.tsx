...
import BannerNewsBlock from "./BannerNewsBlock";
import Pagination from "./Pagination";

interface PaginatedSubcategoryContentProps {
  subcategoryValue: string;
  basePath: string;
  initialPosts: Post[];
  initialTotalPages: number;
  initialBannerNews: Post[];
}
      ...
      <div className="p-6 space-y-4">
        ...
        {article.data.tags.join(", ") && (
          <div className="flex items-center gap-2 pt-2 border-t border-default">
            ...
            <span className="text-xs text-gray-500 truncate">{article.data.tags.join(", ")}</span>
          </div>
        )}
      </div>
    </article>
...
export default function PaginatedSubcategoryContent({
  subcategoryValue,
  basePath,
  initialPosts,
  initialTotalPages,
  initialBannerNews,
}: PaginatedSubcategoryContentProps) {
  // State seeds from server-rendered props so hydration matches the static
  // (ISR-cached) markup exactly — any URL-driven correction happens only in
  // the effect below, after hydration, to avoid a mismatch.
  const [page, setPage] = useState(1);
  const [posts, setPosts] = useState(initialPosts);
  const [totalPages, setTotalPages] = useState(initialTotalPages);
  const [bannerNews, setBannerNews] = useState(initialBannerNews);
  ...
  async function fetchPage(newPage: number) {
    ...
    try {
      const res = await fetch(
        `/api/subcategory-posts?subcategory=${encodeURIComponent(subcategoryValue)}&page=${newPage}`,
      );
      const data = await res.json();
      setPosts(data.posts ?? []);
      setTotalPages(data.totalPages ?? 1);
      setBannerNews(data.bannerNews ?? []);
      setPage(newPage);
    } catch {
  ...
  const regularArticles = posts.filter((p) => !p.data.is_featured && !p.data.is_breaking_news);

  return (
    <div className={loading ? "opacity-60 transition-opacity" : "transition-opacity"}>
      {bannerNews.length > 0 && <BannerNewsBlock posts={bannerNews} />}
        ...
        <section className="mb-8">
          <div className="bg-gradient-to-r from-red-600 to-red-800 border border-red-500 rounded-lg p-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-white font-bold text-lg uppercase tracking-wider">Breaking News</span>
            </div>
        ...
        {regularArticles.length > 0 ? (
          <section className="mb-12">
            <div className="flex items-center gap-4 mb-8">
              ...
              <h2 className="text-3xl font-roboto font-bold text-white">Latest Articles</h2>
            </div>
          ...
          posts.length === 0 && (
            <div className="text-center py-16 text-gray-400">No articles found on this page.</div>
          )
        )}
