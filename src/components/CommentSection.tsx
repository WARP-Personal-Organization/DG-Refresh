...
function validate(n: string, e: string, t: string) {
...
interface CommentItemProps {
  ...
  onReplyStateChange: (updater: (s: ReplyState) => ReplyState) => void;
  submitting: boolean;
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-6 text-sm mb-4">
            <button
              type="button"
              aria-label={liked.has(node.id) ? "Unlike comment" : "Like comment"}
            ...
            <button
              type="button"
              aria-label={disliked.has(node.id) ? "Remove dislike" : "Dislike comment"}
              onClick={() => onToggleDislike(node.id)}
          ...
          {replyingTo === node.id && (
            <div className="mt-6 p-6 bg-gray-800/50 rounded-lg border border-gray-700">
              ...
              <form onSubmit={(e) => { e.preventDefault(); onReplySubmit(node.id); }} className="space-y-4">
                <div className="grid md:grid-cols-2 gap-4">
                  <input
                    type="text"
                    value={replyState.name}
                    onChange={(e) => onReplyStateChange((s) => ({ ...s, name: e.target.value }))}
                    placeholder="Name: *"
                  ...
                  <input
                    ...
                    onChange={(e) => onReplyStateChange((s) => ({ ...s, email: e.target.value }))}
                    placeholder="Email: *"
                    className="w-full p-3 bg-gray-800 border border-gray-600 rounded text-gray-200 placeholder-gray-400 focus:border-[#fcee16] focus:outline-none font-open-sans"
                    required
                  />
                ...
                <textarea
                  ...
                  onChange={(e) => onReplyStateChange((s) => ({ ...s, text: e.target.value }))}
...
const CommentSection = ({ postId, initialComments = [] }: CommentSectionProps) => {
  ...
  const toggleReply = (id: number) => setReplyingTo(replyingTo === id ? null : id);

  const cancelReply = () => { setReplyingTo(null); setReplyState(emptyReply()); };
