...
import { Clock, Tag, TrendingUp, Eye } from 'lucide-react';
...
const getCategoryInfo = (category: "news" | "sports" | "business" | "featured" | null | undefined) => {
  const categoryMap = {
    ...
    "featured": { 
      name: "Featured", 
      color: "bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-400 text-black",
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {topStories.map((story, index) => {
            ...
            const publishDate = story.data.published_date || story.data.updated_date;
            ...
            return (
              ...
              >
                <article className={`${backgroundVariations[index % 4]} ${hoverBackgrounds[index % 4]} rounded-xl overflow-hidden border-2 border-gray-800 hover:border-yellow-500/60 transition-all duration-500 hover:shadow-2xl hover:shadow-yellow-500/20 group-hover:border-yellow-400`}>
                  ...
                  <div className="relative aspect-[16/10] overflow-hidden">
                    {storyImage?.url ? (
                      ...
                      <div className="w-full h-full bg-gradient-to-br from-gray-800 via-gray-900 to-black flex items-center justify-center">
                        <div className="text-center">
                          <Tag size={32} className="mx-auto text-yellow-400/50 mb-2" />
                    ...
                    <div className="absolute top-3 left-3">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold tracking-wider uppercase shadow-lg backdrop-blur-sm ${categoryInfo.color}`}>
                        {categoryInfo.name}
                      </span>
                    </div>
                  ...
                  <div className="p-6 space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      <span className={`text-xs font-bold uppercase tracking-wider ${categoryInfo.textColor} group-hover:text-yellow-300 transition-colors duration-200`}>
                        {categoryInfo.name}
                      </span>
                      {story.data.is_featured && (
        ...
        {topStories.length === 0 && (
          <div className="text-center py-16">
            <div className="bg-gradient-to-br from-gray-900 via-black to-gray-950 border-2 border-gray-800 rounded-xl p-8 max-w-md mx-auto">
              ...
              <h3 className="text-xl font-bold text-white mb-2">No Stories Available</h3>
              <p className="text-gray-400">Check back soon for the latest local news updates.</p>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-center mt-16 relative">
          <div className="absolute inset-0 flex items-center">
            <div className="w-full border-t border-gradient-to-r from-transparent via-yellow-500/30 to-transparent"></div>
          </div>
